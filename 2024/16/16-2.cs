...
// Manually calculate answer from almost-correct output
Console.WriteLine($"Tiles = {tiles}");

void ReverseRoute(Node current)
{
    visited.Add(current);
    tiles++;
    //Console.WriteLine($"Reversing: {current.y},{current.x}");
    route.Add((current.y,current.x));
    if(current.previous == null)
        return;
    foreach(Node previous in current.previous)
    {
        if(!visited.Contains(previous))
            ReverseRoute(previous);
    }
}
        ...
        foreach(Node neighbour in current.neighbours)
        {
            if(neighbour.x == current.x && current.entered_angle == '|')
            {
                if(current.distance + 1 < neighbour.distance)
                {
                    ...
                    if(neighbour.previous.Count == 0)
                        neighbour.previous.Add(current);
                    else
                    {
                        for(int i = 0; i < neighbour.previous.Count; i++)
                        {
                            if(neighbour.previous[i].distance > current.distance)
                            {
                                neighbour.previous.RemoveAt(i);
                                i--;
                            }
                        }
                        neighbour.previous.Add(current);
            ...
            if(neighbour.y == current.y && current.entered_angle == '-')
            {
                if(current.distance + 1 < neighbour.distance)
                {
                    neighbour.distance = current.distance + 1;
            ...
            if(current.entered_angle != neighbour.entered_angle && current.distance <= neighbour.distance + 1000)
            {
                if(neighbour.previous.Count == 0)
                    neighbour.previous.Add(current);
                else
